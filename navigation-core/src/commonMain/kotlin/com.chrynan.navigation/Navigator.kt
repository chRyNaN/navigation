...
interface Navigator<Destination : NavigationDestination, Context : NavigationContext<Destination>> {
    ...
    /**
     * Navigates to the provided [event]. Currently, this default implementation delegates to the appropriate [goBack],
     * [goUp], and [goTo] function depending on the provided [event] value.
     *
     * @param [event] The [NavigationEvent] that represents the navigation action to be performed.
     */
    fun navigate(event: NavigationEvent<Destination>) {
        when (event) {
            is NavigationEvent.Back -> goBack()
            is NavigationEvent.Up -> goUp()
            is NavigationEvent.To -> goTo(destination = event.destination, strategy = event.strategy)
        }
    }
     ...
     */
    fun goTo(destination: Destination, strategy: StackDuplicateContentStrategy)
...
abstract class BaseNavigatorImpl<Destination : NavigationDestination, Context : NavigationContext<Destination>, State : BaseNavigatorStateImpl<Destination, Context>>(
    ...
    final override fun goTo(destination: Destination, strategy: StackDuplicateContentStrategy) {
        ...
        if (destination == currentKeyStack.lastOrNull()) return
    ...
    final override fun goBack(): Boolean {
